...
import type { Escala, EscalaItem, Presenca } from '../types'
import Badge from '../components/common/Badge'
import LoadingSpinner from '../components/common/LoadingSpinner'
import { formatDataLong, formatHorario } from '../lib/dateUtils'

type PresencaStatus = Presenca['status']

const PRESENCA_OPTIONS: { value: PresencaStatus; label: string; icon: React.ReactNode; activeClass: string; hoverClass: string }[] = [
  {
    value: 'confirmado',
    label: 'Confirmado',
...
export default function EscalaView() {
  ...
  async function handlePresenca(item: EscalaItem, status: PresencaStatus) {
    try {
      await api.put(`/escala-itens/${item.id}/presenca`, { status })
        ...
        <div className="divide-y divide-gray-100">
          {(escala.escala_itens ?? escala.itens ?? []).length === 0 ? (
            ...
            (escala.escala_itens ?? escala.itens ?? []).map((item, index) => {
              const currentStatus = item.presenca?.status
              return (
                <div
                  key={item.id}
                  className="flex items-start sm:items-center gap-4 px-5 py-4 hover:bg-gray-50/60 transition-colors"
                >
                  <div className="w-8 h-8 rounded-full bg-wine-900 text-gold-400 flex items-center justify-center font-bold text-xs flex-shrink-0 mt-0.5 sm:mt-0">
                    {index + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-gray-500 font-medium mb-0.5">{item.funcao_label || item.funcao?.titulo}</div>
                    ...
                    <div className="flex-shrink-0 flex flex-wrap gap-1">
                      {PRESENCA_OPTIONS.map((opt) => (
                        <button
                          ...
                          title={opt.label}
                          className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold border transition-all duration-200 ${
